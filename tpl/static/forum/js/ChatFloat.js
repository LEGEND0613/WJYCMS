var SocketIp = "kf.dogcms.com";
var TimeInterval;
var getType = 1;
var ajaxAddress = "http://kf.dogcms.com/";
...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.dogcmsid && obj.dogcmsid.length > 0) {
                    t.initialize(options, obj.dogcmsid, obj.msgcount, obj.socket);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"dogcmsid":"' + obj.dogcmsid + '","socket":' + obj.socket + '}');
                }
            }
        }, function (status) { });
    ...
    SocketMessage: function (AId, dogcmsid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatSocketMessage&dogcmsid=' + dogcmsid + "&AId=" + AId, true);
    },
    StartPolling: function (AId, dogcmsid) {
        getType = 2;
        clearInterval(TimeInterval);
        TimeInterval = setInterval(GetPolling, 10000);
        this.GetPolling(AId, dogcmsid);
    },
    GetPolling: function (AId, dogcmsid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatPolling&dogcmsid=' + dogcmsid + "&AId=" + AId, true);
    },
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId && objStorage.dogcmsid) {
                var socket = 0;
                if (objStorage.socket) {
                    socket = objStorage.socket;
                }
                e.initialize(options, objStorage.dogcmsid, 0, socket);
                Tools.SocketMessage(options.AId, objStorage.dogcmsid);
            }
            else {
...
ChatFloat.prototype = {
    initialize: function (options, dogcmsid, msgcount, socket) {
        var e = this;
        ...
        if (socket == 1) {
            Tools.loadjscssfile("http://" + SocketIp + ":8889/socket.io/socket.io.js", "js", function () { e.socketconnect(options, dogcmsid); });
        }
    ...
    socketconnect: function (options, dogcmsid) {
        try {
            ...
            socket.on("connect", function () {
                ...
                socket.on('dataChange', function (json) {
                    if (json) {
                        if (json.msgCount == 1) {
                            Tools.SocketMessage(options.AId, dogcmsid);
                        }
                    }
                });
            });
