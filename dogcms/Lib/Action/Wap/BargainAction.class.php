...
class BargainAction extends WapAction
{
	...
	public function home()
	{
		...
		foreach ($bargain_list as $k => $v) {
			...
			$where_order_paynum['bargain_id'] = $v['dogcms_id'];
	...
	public function dao()
	{
		...
		$where_order1['dogcms_id'] = (int) $_GET['orderid'];
		...
		if (($order1 == '') || ($order1['wecha_id'] == $this->wecha_id)) {
			...
			if ($order2 == '') {
				...
				$where_kanuser['orderid'] = $order2['dogcms_id'];
			...
			if ($kanuser == '') {
				...
				$where_kanuser2['orderid'] = $order1['dogcms_id'];
	...
	public function operate()
	{
		...
		case 'firstdao':
			...
			if ($order == '') {
				...
				$where_order_orderid['dogcms_id'] = $id_order;
		...
		case 'friendkan':
			$where_order['token'] = $this->token;
			$where_order['dogcms_id'] = (int) $_GET['orderid'];
			$order = $this->m_order->where($where_order)->find();
			$where_kanuser['token'] = $this->token;
			...
			if ($kanuser == '') {
				...
				if ($order['bargain_nowprice'] == $bargain['minimum']) {
					...
					$add_kanuser['bargain_id'] = (int) $_GET['id'];
					$add_kanuser['orderid'] = $order['dogcms_id'];
					$add_kanuser['friend'] = $this->wecha_id;
	...
	public function payuserinfo()
	{
		...
		if ($_GET['orderid'] == '') {
			...
			$id_order = $this->m_order->add($add_order);
			$where_order['token'] = $this->token;
			$where_order['dogcms_id'] = $id_order;
			$randnum = rand(1000, 9999);
			$save_order_orderid['orderid'] = $id_order . 'bargain' . time() . $randnum;
	...
	public function dobuy()
	{
		...
		$update_order = $this->m_order->where($where_order)->save($save_order);
		$order = $this->m_order->where($where_order)->find();
		$where['dogcms_id'] = $order['bargain_id'];
		$where['token'] = $this->token;
		$inventory = $this->m_bargain->where($where)->getField('inventory');

		if ($inventory < 1) {
			$this->error('库存不够了', U('Bargain/home', array('token' => $this->token)));
			exit();
		}
		...
		$this->redirect('Alipay/pay', array('token' => $this->token, 'price' => (int) $order2['bargain_nowprice'], 'wecha_id' => $this->wecha_id, 'from' => 'Bargain', 'orderid' => $order['orderid'], 'single_orderid' => $order['orderid'], 'notOffline' => 1));
	}
