...
class IndexAction extends WapAction{
	...
	public function _initialize(){
		...
		if($_SERVER['HTTP_REFERER'] != "" && strpos($_SERVER['HTTP_REFERER'],'memberReg') === false && strpos($_SERVER['HTTP_REFERER'],'memberLogin') === false && strpos($_SERVER['HTTP_REFERER'],'PhoneVerify') === false){
			session('R' , $_SERVER['HTTP_REFERER']);
		}
		$this->R = session('R');
	}
	...
	public function content($contid='',$cid=''){
		...
		//获取模板
			include('./dogcms/Lib/ORG/cont.Tpl.php');
