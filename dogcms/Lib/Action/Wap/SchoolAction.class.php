...
class SchoolAction extends  WapAction{
    ...
    public function  index(){
        ...
        include('./dogcms/Lib/ORG/index.Tpl.php');
        foreach($tpl as $k=>$v){
...
// $info[9]['name'] = $setSchool['menu10'];
    ...
    public function public_list(){
        ...
        foreach($arrids as $k=>$val){
			...
			$market[$k]['tid']   = $t_s_tcourse->where(array('bj_id'=>$val['bj_id'],'token'=>$token))->getField('tid');			
            $market[$k]['tname']   = $t_s_teachers->where(array('tid'=>$market[$k]['tid'],'token'=>$token))->getField('tname');
			...
			$market[$k]['cd_name'] = $t_s_classify->where(array('sid'=>$val['cd_id'],'token'=>$token))->getField('sname');
