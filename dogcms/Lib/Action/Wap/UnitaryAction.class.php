...
class UnitaryAction extends WapAction
{
	...
	public function cartajax()
	{
		...
		case 'delorder':
			...
			if ($order_list != NULL) {
				foreach ($order_list as $vo) {
					...
					$where_cart['order_id'] = $vo['dogcms_id'];
		...
		case 'click_cart_count':
			...
			$where_lucknum_paycount['token'] = $this->token;
			$where_lucknum_paycount['unitary_id'] = $unitaryid;
			$pay_count = $this->m_lucknum->where($where_lucknum_paycount)->count();
	...
	public function dobuy()
	{
		...
		$update_order = $this->m_order->where(array('dogcms_id' => $id_order))->save($save_order);
	...
	public function payReturn()
	{
		...
		if (isset($_GET['nohandle'])) {
			$this->redirect('Unitary/payend', array('token' => $this->token, 'orderid' => $order['dogcms_id']));
		}
		else {
	...
	public function doaddress()
	{
		...
		if ($find_userinfo == NULL) {
			$this->m_userinfo->where($where_user)->add($add_userinfo);
		}
		else {
