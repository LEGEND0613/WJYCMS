...
class SeckillAction extends WapAction
{
	...
	public function index()
	{
		...
		$_SESSION['suid'] = M('seckill_users')->where(array('user_wechaid' => $this->wecha_id))->getField('user_id');
	...
	public function pay()
	{
		if (IS_POST) {
			...
			$wecha_id = (isset($_POST['wecha_id']) ? trim($_POST['wecha_id']) : $this->wecha_id);
			$price1 = (isset($_POST['price']) ? (double) $_POST['price'] : 0);
			$tran = (isset($_POST['tran']) ? (double) $_POST['tran'] : 0);
			$orderName = (isset($_POST['title']) ? trim($_POST['title']) : time());
	...
	public function my_cart()
	{
		...
		$model = new Model();
		$sql = 'select a.shop_name ,a.shop_id ,a.shop_price, a.shop_img, b.orderid, b.paid, b.price,b.paytype,b.book_id from ' . C('DB_PREFIX') . 'seckill_base_shop as a left join ' . C('DB_PREFIX') . 'seckill_book as b on a.shop_id = b.book_sid where b.book_uid = ' . $uid . ' and b.token = \'' . $this->token . '\' and b.book_aid = ' . $this->action_id . ' order by b.paid , b.book_time desc';
