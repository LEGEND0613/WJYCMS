...
class HotelsAction extends WapAction {
	...
	public function index() {
		$company = M('Company')->where("`token`='{$this->token}' AND ((`isbranch`=1 AND `display`=1) OR `isbranch`=0) AND (`business_type` LIKE '%Hotels%' OR `business_type`='')")->select();
		if (count($company) == 1) {
...
// 					}
					...
					$model = new templateNews();
					$model->sendTempMsg('TM00184', array('href' => U('Hotels/my',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'cid' => $this->_cid), true, false, true), 'wecha_id' => $this->wecha_id, 'first' => '预订房间提醒', 'ordertape' => date("Y年m月d日H时i分s秒"), 'ordeID' => $orderid, 'remark' => '预订房间成功，感谢您的光临，欢迎下次再次光临！'));
					if ($_POST['paymode'] == 4 && $this->fans['balance']) {
						$this->success('正在提交中...', U('CardPay/pay',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'from'=> 'Hotels', 'orderName' => $orderid, 'single_orderid' => $orderid, 'price' => $_POST['price'])));
					} elseif ($config['open']) {
						$this->success('正在提交中...', U('Alipay/pay',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'from'=> 'Hotels', 'orderName' => $orderid, 'single_orderid' => $orderid, 'price' => $_POST['price'])));
					} else {
						$this->success('预定成功,进入您的订单页', U('Hotels/my',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'cid' => $this->_cid)));
					}
				} else {
					$this->error('操作失败');
				}
