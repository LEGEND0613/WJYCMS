...
class EstateAction extends WapAction{
    ...
    public function index(){
        ...
        for ($i=0; $i <5; $i++) { 
            if($this->es_data['slide'.($i+1)] != ''){
                 ...
                 $flash[$i]['img'] = $this->es_data['slide'.($i+1)];
            }  
        }
    ...
    public function _getNav(){
        ...
        foreach ($nav as $key => $value) {
            ...
            $info[$key]['url']  = str_replace(array('{id}','{wecha_id}','{siteUrl}','{rid}'),array($this->es_data['id'],$this->wecha_id,$this->siteUrl,$this->es_data['res_id']),$info[$key]['url']);
