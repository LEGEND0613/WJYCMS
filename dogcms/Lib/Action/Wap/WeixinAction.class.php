...
class WeixinAction extends BaseAction{
	...
	private function code($orderid, $price)
	{
		...
		$unifiedOrder->setParameter("attach", 'token=' . $_GET['token'] . '&wecha_id=' . $_GET['wecha_id'] . '&from=' . $_GET['from'] . '&pl=' . $this->_issystem);//附加数据
	...
	public function new_pay()
	{
		...
		//判断是否已经支付过
		if($orderInfo['paid']) {
			$returnUrl = $this->siteUrl.'/index.php?g=Wap&m='.$_GET['from'].'&a=payReturn&nohandle=1&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid.'&price=' . $price;//自行增加&price
		...
		if ($this->_issystem) {
			...
			//使用统一支付接口
			$unifiedOrder = new UnifiedOrder_pub($this->payConfig['new_appid'], $this->payConfig['mchid'], $this->payConfig['key'], $this->payConfig['appsecret']);	
			$unifiedOrder->setParameter("openid", $this->wecha_id);//商品描述
			$unifiedOrder->setParameter("body", $orderid);//商品描述
			//自定义订单号，此处仅作举例
		...
		$returnUrl = $this->siteUrl.'/index.php?g=Wap&m='.$from.'&a=payReturn&nohandle=1&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid.'&price=' . $price;//自行增加&price
		$this->assign('returnUrl',$returnUrl);
		//$this->display('new_pay');
		...
		if ($this->_issystem) {
			$message .= '<ul class="round" id="cross_pay"><li class="title mb" style="text-align:center"><span class="none">微信扫描支付</span></li><li class="nob" style="margin-bottom: 18px;"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="kuang"><tr><td style="text-align:center" ><img src="' . U('Weixin/qrcode', array('url' => $url)) . '" height="200" id="show_success"></td></tr><tr><td style="text-align:center">长按图片[识别二维码]付款</td></tr></table></li><li class="mb" style="text-align:center;margin-top: 20px;border-top: 1px solid #C6C6C6;" id="success"><span class="none"><a href="{dogcms:$returnUrl}" style="color:#459ae9">我已经支付成功</a></span></li></ul>';
		} else {
