...
class CutpriceAction extends WapAction
{
	...
	{
		$where['token'] = $this->token;
		$where['dogcms_id'] = intval($_GET['id']);
		$cutprice = $this->m_cutprice->where($where)->find();
	...
	public function dobuy()
	{
		...
		if ($order_is != '') {
			...
			if ($order_is2 != '') {
				...
				$update_order = $this->m_order->where(array('token' => $this->token, 'dogcms_id' => $id_order))->save(array('orderid' => $orderid));
	...
	{
		$where_order['token'] = $this->token;
		$where_order['dogcms_id'] = $_GET['orderid'] * 1;
