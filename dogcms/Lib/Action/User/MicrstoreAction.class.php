...
class MicrstoreAction extends UserAction
{
	...
	public function _initialize()
	{
		...
		if (updateSync::getIfWeidian()) {
			...
			$this->SALT = C('encryption') ? C('encryption') : 'dogcms';
		}
		else {
	...
	public function withdraw()
	{
		...
		if ($return['error_code'] == 0) {
			...
			foreach ((array) $return['withdrawals'] as $key => $val) {
				$insert_data['dogcms_id'] = $val['dogcms_id'];
				...
				$exists = M('microsoft_withdraw')->where(array('dogcms_id' => $val['dogcms_id']))->find();

				if ($exists) {
					$update = M('microsoft_withdraw')->where(array('dogcms_id' => $val['dogcms_id']))->save($insert_data);
				}
				else {
	...
	public function edit_withdraw()
	{
		if (IS_POST) {
			...
			$dogcms_id = $this->_post('dogcms_id', 'intval');
			...
			if (empty($status) || empty($dogcms_id)) {
