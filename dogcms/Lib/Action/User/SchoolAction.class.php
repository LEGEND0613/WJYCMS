...
class SchoolAction extends UserAction{
    ...
    public function index(){
        $arrType = array('semester','theclass','score','subject','timeframe','week','saddrs','syllabus','syllabus_details','syllabus_details_list');
    ...
    public function del_item(){
        $arrType = array('students','assess','curriculum','scoresearch','semester','theclass','score','subject','timeframe','','saddrs','syllabus','syllabus_details');
        ...
        if(!in_array($type,$arrType)){
         ...
         //$type   = filter_var($this->_get('deltype'),FILTER_SANITIZE_STRING);
          if($type == 'students'){
            ...
            M('school_adress')->where(array('token'=>session('token'),'tid'=>$id))->delete();
		  }  elseif($type == 'syllabus'){
            M('school_syllabus')->where(array('token'=>session('token'),'id'=>$id))->delete();

          } elseif('scoresearch' == $type){
            M('school_score')->where(array('token'=>session('token'),'id'=>$id))->delete();
          }elseif('syllabus_details' == $type){
            M('school_syllabus_details')->where(array('token'=>session('token'),'id'=>$id))->delete();
          }
    ...
    public function curriculum(){
        ...
        foreach($arrids as $k=>$val){
			...
			//$market[$k]['cd_name'] = $t_s_classify->where(array('sid'=>$val['cd_id'],'token'=>session('token')))->getField('sname');
        }
		//print_r($market);
	 ...
	 //public function curriculum_add(){
		...
		//判断是否为此班级的学生
		 ...
		 $bj_id =filter_var($this->_get('bj_id'),FILTER_VALIDATE_INT);
		 $xq_id =filter_var($this->_get('xq_id'),FILTER_VALIDATE_INT);
         if(isset($bj_id) && $bj_id!= '' && isset($xq_id) && $xq_id!= '')
         {
         	    $where = array('token'=>session('token'),'bj_id'=>$bj_id,'xq_id'=>$xq_id);
                //$where['bj_id'] = $bj_id;
        ...
        $where2 = array('token'=>session('token'),'type'=>'subject');
        $li_subject = $t_s_classify->where($where2)->order('ssort DESC')->select();
        $where_s = array('token'=>session('token'),'type'=>'semester');
        $li_semester = $t_s_classify->where($where_s)->order('ssort DESC')->select();
        //$where_t = array('token'=>session('token'),'type'=>'timeframe');
        //$li_timeframe = $t_s_classify->where($where_t)->order('ssort DESC')->select();
		//$where_t = array('token'=>session('token'),'type'=>'adress');
        //$li_adress = $t_s_classify->where($where_t)->order('ssort DESC')->select();
        $t_s_tcourse = M('school_tcourse');
        $t_s_teachers = M('school_teachers');
        $where4 = array('token'=>session('token'));
        $li_teachers = $t_s_teachers->where($where4)->order('sort DESC')->select();
         if(IS_POST){
            $_POST['token'] = session('token');                   
                    if($id=$t_s_tcourse->data($_POST)->add()){
                        $this->success('添加成功',U('School/curriculum',array('token'=>session('token'))));exit;
                    }else{
                        exit($this->error('服务器繁忙,添加失败,请稍候再试'));
                    }                        
        }

        $this->assign('li_teachers',$li_teachers);
        $this->assign('li_theclass',$li_theclass);
        $this->assign('li_semester',$li_semester);
        //$this->assign('li_timeframe',$li_timeframe);
        $this->assign('li_subject',$li_subject);
		//$this->assign('li_adress',$li_adress);
        $this->display();
    }
	  */
     public function curriculum_add(){
		...
		//判断是否为此班级的学生
         ...
         if(isset($bj_id) && $bj_id!= '')
         {
                ...
                $this->assign('students',$students);			   
				$where1 = array('token'=>session('token'),'type'=>'theclass','sid'=>$bj_id);
				$li_theclass = $t_s_classify->where($where1)->order('ssort DESC')->select();  
         }
	    ...
	    //$t_s_teachers = M('school_teachers');
        //$where4 = array('token'=>session('token'));
        //$li_teachers = $t_s_teachers->where($where4)->field('tid,tname')->find();
        ...
        if(isset($getid) && $type == 'edit' && $getid != ''){
            $s_tcourse = $t_s_tcourse->where(array('id'=>$getid,'token'=>session('token')))->find();
             $this->assign('tcourse',$s_tcourse);
        ...
        if(IS_POST){
            ...
            if($t_s_tcourse->create()!=false){
                if($type == 'edit' && $getid != ''){
                    $where5 = array('token'=>session('token'),'id'=>$getid);
                    if($t_s_tcourse->where($where5)->save($_POST)){
                        $this->success('修改成功',U('School/curriculum',array('token'=>session('token'))));exit;
                    }else{
                        $this->error('服务器繁忙,修改失败,请稍候再试');exit;
                    }
                }else{
    ...
    public function adress(){
        $t_s_adress = M('school_adress');
        $where =  array('token'=>session('token'));
        $count      = $t_s_adress->where($where)->count();
        $Page       = new Page($count,20);
        $show       = $Page->show();
        $adress = $t_s_adress->where($where)->order('tid DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('page',$show);       
    ...
    public function adress_add(){
        ...
        $t_s_adress = M('school_adress');
        $where = array('token'=>session('token'));
        $s_adress = $t_s_adress->where($where)->select();
        if(IS_POST){
            $_POST['token'] = session('token');
            $_tid =filter_var($this->_post('tid'),FILTER_VALIDATE_INT);
            ...
            $info = filter_var($this->_post('info'),FILTER_SANITIZE_STRING);
            if(empty($info)){
               exit($this->error('亲,教室简介不能为空!',U('School/adress',array('token'=>session('token')))));
            }

            if($t_s_adress->create()!=false){
                if(isset($_tid) && $_tid != ''){
                    $where3 = array('token'=>session('token'),'tid'=>$_tid);
                    if($t_s_adress->where($where3)->save($_POST)){
                        $this->success('修改成功',U('School/adress',array('token'=>session('token'))));exit;
                    }else{
                        $this->error('服务器繁忙,修改失败,请稍候再试',U('School/adress',array('token'=>session('token'))));exit;
                    }

                }else{
                    $_POST['createtime'] = time();
                    if($id=$t_s_adress->data($_POST)->add()){
                        $this->success('添加成功',U('School/adress',array('token'=>session('token'))));exit;
                    }else{
                        $this->error('服务器繁忙,添加失败,请稍候再试',U('School/adress',array('token'=>session('token'))));exit;
                    }
                }
            }else{
                $this->error($t_s_adress->getError());exit;
            }
        }
        $get_tid =filter_var($this->_get('tid'),FILTER_VALIDATE_INT);
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
        if(isset($get_tid) && $get_tid != '' && $type == 'edit'){
             ...
             $where_1 = array('token'=>session('token'),'tid'=>$get_tid);
             $s_adress = $t_s_adress->where($where_1)->find();
             $this->assign('type','edit');
        }
        $this->assign('s_adress',$s_adress);
        $this->display();
    }
    public function syllabus(){
        $t_s_syllabus = M('school_syllabus');
        $where =  array('token'=>session('token'));
        $count      = $t_s_syllabus->where($where)->count();
        $Page       = new Page($count,20);
        $show       = $Page->show();
        $arrids = $t_s_syllabus->where($where)->order('xq_id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
        $t_s_teachers = M('school_teachers');
        $t_s_classify = M('school_classify');
		$t_s_tcourse = M('school_tcourse');
        $market = array();
        foreach($arrids as $k=>$val){
            $market[$k]['id']      = $val['id'];
            $market[$k]['xq_id']   = $val['xq_id'];
			$market[$k]['bj_id']   = $val['bj_id'];
			$market[$k]['sd_id']   = $val['sd_id'];
			$market[$k]['cd_id']   = $val['cd_id'];
			$market[$k]['bz_id']   = $val['bz_id'];
			$market[$k]['tid']   = $t_s_tcourse->where(array('bj_id'=>$val['bj_id'],'token'=>session('token')))->getField('tid');			
            $market[$k]['tname']   = $t_s_teachers->where(array('tid'=>$market[$k]['tid'],'token'=>session('token')))->getField('tname');
			$market[$k]['bj_name'] = $t_s_classify->where(array('sid'=>$val['bj_id'],'token'=>session('token')))->getField('sname');
            $market[$k]['bz_name'] = $t_s_teachers->where(array('tid'=>$val['bz_id'],'token'=>session('token')))->getField('tname');
            //$market[$k]['km_name'] = $t_s_classify->where(array('sid'=>$val['km_id'],'token'=>session('token')))->getField('sname');
            $market[$k]['xq_name'] = $t_s_classify->where(array('sid'=>$val['xq_id'],'token'=>session('token')))->getField('sname');
            $market[$k]['sd_name'] = $t_s_classify->where(array('sid'=>$val['sd_id'],'token'=>session('token')))->getField('sname');
			$market[$k]['cd_name'] = $t_s_classify->where(array('sid'=>$val['cd_id'],'token'=>session('token')))->getField('sname');
    ...
    public function syllabus_add(){
       ...
       // $jointable = C('DB_PREFIX') . 'dish_order';
		...
		$t_s_tcourse = M('school_tcourse');
		$sql = 'select tab.*,tname from
         ( select DISTINCT (b.bj_id), b.tid, a.sname  from ' . C('DB_PREFIX') .'school_tcourse as b  left join  '. C('DB_PREFIX').'school_classify as a   on a.sid = b.bj_id and b.token= a.token  where a.token= \''."$token1".'\' and  a.type=\'theclass\' GROUP BY b.tid, a.sname
         )tab left join '. C('DB_PREFIX').'school_teachers as t  on t.tid=tab.tid ';		
		$Model = new Model();
		$li_tcourse = $Model->query($sql);	
		//$where_tcourse = array('token'=>session('token'));
        //$li_tcourse = $t_s_tcourse->where($where_tcourse)->select();  
		
        $t_s_teachers = M('school_teachers');
        $where4 = array('token'=>session('token'));
        $li_teachers = $t_s_teachers->where($where4)->field('tid,tname')->order('sort DESC')->select();
        ...
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
        if(isset($getid) && $type == 'edit' && $getid != ''){
            $s_asyllabus = $t_s_syllabus->where(array('id'=>$getid,'token'=>session('token')))->find();			
			 $this->assign('tcourse',$s_asyllabus);
			 $this->assign('type','edit');	
        }
        if(IS_POST){
              $_POST['token'] = session('token');
              $istoken = trim(filter_var($this->_post('istoken'),FILTER_SANITIZE_STRING));
                if($type == 'edit' && $getid != ''){
                    $where5 = array('token'=>session('token'),'id'=>$getid);
                    if($t_s_syllabus->where($where5)->save($_POST)){
                        $this->success('修改成功',U('School/syllabus',array('token'=>session('token'))));exit;
                    }else{
                        $this->error('服务器繁忙,修改失败,请稍候再试');exit;
                    }

                }else{
                    if($id=$t_s_syllabus->data($_POST)->add()){
                        $this->success('添加成功',U('School/syllabus',array('token'=>session('token'))));exit;
                    }else{
                        exit($this->error('服务器繁忙,添加失败,请稍候再试'));
                    }
                }
        ...
        $this->assign('li_teachers',$li_teachers);
        $this->assign('li_tcourse',$li_tcourse);
	...
	public function syllabus_details_list(){
		        $t_s_syllabus_details = M('school_syllabus_details');
		        $where =  array('token'=>session('token'));
		        $count      = $t_s_syllabus_details->where($where)->count();
		        $Page       = new Page($count,20);
		        $show       = $Page->show();
		        $arrids = $t_s_syllabus_details->where($where)->order('datetime,sy_id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
		        $t_s_teachers = M('school_teachers');
		        $t_s_classify = M('school_classify');
				$t_s_tcourse = M('school_tcourse');
				$t_s_syllabus = M('school_syllabus');
		        ...
		        foreach($arrids as $k=>$val){		        	
		            $market[$k]['id']      = $val['id'];
					$market[$k]['datetime']= $val['datetime'];
		            $market[$k]['sy_id']   = $val['sy_id'];
					$market[$k]['isnot']   = $val['isnot'];
					$market[$k]['bj_id']   =$t_s_syllabus->where(array('id'=>$val['sy_id'],'token'=>session('token')))->getField('bj_id');			
					$market[$k]['sd_id']   = $t_s_syllabus->where(array('id'=>$val['sy_id'],'token'=>session('token')))->getField('sd_id');	
					$market[$k]['cd_id']   = $t_s_syllabus->where(array('id'=>$val['sy_id'],'token'=>session('token')))->getField('cd_id');			
					$market[$k]['bz_id']   = $t_s_syllabus->where(array('id'=>$val['sy_id'],'token'=>session('token')))->getField('bz_id');	
					$market[$k]['xq_id']   = $t_s_syllabus->where(array('id'=>$val['sy_id'],'token'=>session('token')))->getField('xq_id');								
					$market[$k]['tid']   = $t_s_tcourse->where(array('bj_id'=>$market[$k]['bj_id'],'token'=>session('token')))->getField('tid');			
		            $market[$k]['tname']   = $t_s_teachers->where(array('tid'=>$market[$k]['tid'],'token'=>session('token')))->getField('tname');
					$market[$k]['bj_name'] = $t_s_classify->where(array('sid'=>$market[$k]['bj_id'],'token'=>session('token')))->getField('sname');
		            $market[$k]['bz_name'] = $t_s_teachers->where(array('tid'=>$market[$k]['bz_id'],'token'=>session('token')))->getField('tname'); 
		            $market[$k]['xq_name'] = $t_s_classify->where(array('sid'=>$market[$k]['xq_id'],'token'=>session('token')))->getField('sname');
		            $market[$k]['sd_name'] = $t_s_classify->where(array('sid'=>$market[$k]['sd_id'],'token'=>session('token')))->getField('sname');
					$market[$k]['cd_name'] = $t_s_classify->where(array('sid'=>$market[$k]['cd_id'],'token'=>session('token')))->getField('sname');
		        }
				$this->assign('page',$show);  		
		        $this->assign('market',$market);	
		        $this->display();
    ...
    public function syllabus_details_add(){
        date_default_timezone_set('PRC');		
		$t_s_syllabus = M('school_syllabus');
		$t_s_syllabus_details = M('school_syllabus_details');
		$t_s_tcourse = M('school_tcourse');
		$sql = 'SELECT b.*,c.sname as bj_name,t.tname,d.sname as cd_name,e.sname as sd_name from  '. C('DB_PREFIX').'school_syllabus b   left join  '. C('DB_PREFIX').'school_classify  
				as c  on c.sid = b.bj_id and b.token= c.token 
				left join '. C('DB_PREFIX').'school_teachers as t  on t.tid=b.bz_id
 				and b.token= t.token
				left join '. C('DB_PREFIX').'school_classify  d on d.sid = b.cd_id
				and b.token= d.token
				left join '. C('DB_PREFIX').'school_classify  e on e.sid = b.sd_id
				and b.token= e.token';
		$Model = new Model();
		$li_tcourse = $Model->query($sql);	
        $getid = filter_var($this->_get('id'),FILTER_VALIDATE_INT);
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
		$xq_name = filter_var($this->_get('xq_name'),FILTER_SANITIZE_STRING);
        if(isset($getid) && $type == 'details' && $getid != ''&& $xq_name != ''){
        	function addYear(&$date){return $GLOBALS['year'].'-'.$date;}
			function getWorkDays(& $startDate,& $endDate,& $publicHolidays,& $holidayTune){
			      $workDays = array();
			       while(1){
			        $day = date('Y-n-j', $startDate);
			        if(!in_array($day,$publicHolidays)){$workDays[] = $day;				}
			        $startDate += 86400;
			 ...
			 $year = '2016';
			 //添加放假时间
			 $publicHolidays =  array('1-1','2-7','2-8','2-9','2-10','2-11','2-12','2-13','4-4','5-1','5-2','6-9','6-10','6-11','9-15','9-16','9-17','10-1','10-2','10-3','10-4','10-5','10-6','10-7');
			 $publicHolidays = array_map('addYear',$publicHolidays);
 			 //添加工作日调休
			 // define the holiday tune.
			 $holidayTune = array('2-6','2-14','6-12','9-18','10-8','10-9');
			 ...
			 foreach ($workDays as $k => $v) {
               	//echo $k.'=>'.$v.'';
               	if($xq_name==$weekarray[date('w',strtotime($v))])
				{
				  //echo	$weekarray[date('w',strtotime($v))]; 
				  //$data['week']=date('w',$v);	 
                  $data['datetime']  = $v;               
                  $data['token']  = session('token');
                  $data['sy_id']   = $getid;
				  $data['isnot']   = 1;	
				  $ok = $t_s_syllabus_details->where(array('token'=>session('token'),'datetime'=>$v,'sy_id'=>$getid))->delete();				 		
                  $ok = $t_s_syllabus_details->add($data);
            ...
            if($ok){
				 $this->success('添加成功',U('School/syllabus',array('token'=>session('token'))));
                exit;
            }else{
                $this->error('添加失败',U('School/syllabus',array('token'=>session('token'))));
                exit;
            }
		}
	    ...
	    if(IS_POST){
              ...
              $istoken = trim(filter_var($this->_post('istoken'),FILTER_SANITIZE_STRING));
                if($type == 'edit' && $getid != ''){
                    ...
                    if($id=$t_s_syllabus_details->data($_POST)->add()){
                        $this->success('添加成功',U('School/syllabus_details_list',array('token'=>session('token'))));exit;
                    }else{
                        exit($this->error('服务器繁忙,添加失败,请稍候再试'));
                    }
                }
    ...
    public function search(){
        ...
        if(IS_POST){
		    ...
		    $date       = trim(filter_var($_REQUEST['searchkey']['date'],FILTER_SANITIZE_STRING));
            if($key == 'students'){
			  ...
			  elseif($key == 'adress'){
                ...
                if(isset($phone) && $phone != ''){
                    $where['nname'] = array('like',array("%{$phone}%"));
                }
                if(isset($jiontime) && $jiontime != ''){
                    $where['jiontime'] = array('like',array("%{$jiontime}%"));
                }
                if(isset($email) && $email != ''){
                    $where['adress'] = array('like',array("%{$email}%"));
                   // $where['mobile'] = $phone;
                }
		        $count      = $t_s_adress->where($where)->count();
                $Page       = new Page($count,20);
                ...
                $Page       = new Page($count,20);
                $show       = $Page->show();
                $syllabus = $t_s_syllabus->where($where)->order('xq_id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
                $this->assign('page',$show);
                
                $this->assign('syllabus',$syllabus);
                $this->display('syllabus');exit;
              }
			   elseif($key == 'syllabus_details'){
                $t_s_syllabus_details = M('school_syllabus_details');
                if(isset($date) && $date != ''){
                    $where['datetime'] = array('like',array("%{date}%"));
                }
				/*
                if(isset($phone) && $phone != ''){
                    $where['nname'] = array('like',array("%{$phone}%"));
                ...
                $Page       = new Page($count,20);
                $show       = $Page->show();
                $syllabus_details = $t_s_syllabus_details->where($where)->order('datetime DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
                $this->assign('page',$show);
                
                $this->assign('syllabus_details',$syllabus_details);
                $this->display('syllabus_details_list');exit;
              }
