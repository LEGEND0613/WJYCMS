...
class BargainAction extends UserAction
{
	...
	{
		if (IS_POST) {
			$where['dogcms_id'] = $_POST['dogcms_id'];
			$where['token'] = $this->token;
			...
			if ($this->m_bargain->create() != false) {
				...
				$id = $_POST['dogcms_id'];
				$this->handleKeyword($id, 'Bargain', $_POST['keyword'], 0, 0);
				S($_POST['dogcms_id'] . 'bargain' . $this->token, NULL);
	...
	public function order()
	{
		...
		if ($_GET['orderid'] != '') {
			$where_order['dogcms_id'] = $_GET['orderid'] - 10000000;
