	...
	public function members(){
		...
		if ($members){
			...
			foreach ($members as $member){
				...
				$members[$i]['truename'] = $thisUser['truename'] ? $thisUser['truename'] : $thisUser['wechaname'];
...
//删除记录
	...
	public function custom(){
		...
		if (empty($conf)) {
			...
			$fields[] = array('field_id' => 0, 'field_name' => '地址', 'field_option' => '', 'field_type_html' => $this->_formConf('field_type', 'text'), 'field_type' => 'text', 'field_match_html' => $this->_formConf('field_match', '^[\u4e00-\u9fa5\a-zA-Z0-9]+$'), 'field_match' => '^[\u4e00-\u9fa5\a-zA-Z0-9]+$', 'is_show' => 1, 'is_empty' => 0);
			$fields[] = array('field_id' => 0, 'field_name' => '来源渠道', 'field_option' => '', 'field_type_html' => $this->_formConf('field_type', 'text'), 'field_type' => 'text', 'field_match_html' => $this->_formConf('field_match', '^[\u4e00-\u9fa5\a-zA-Z0-9]+$'), 'field_match' => '^[\u4e00-\u9fa5\a-zA-Z0-9]+$', 'is_show' => 1, 'is_empty' => 0);
		} else {
			$fields = M('Member_card_custom_field')->where(array('set_id' => $conf['id'], 'token' => $this->token))->order('field_id ASC')->select();
			if (empty($fields)) {
				$fields[] = array('field_id' => 0, 'field_name' => '地址', 'field_option' => '', 'field_type_html' => $this->_formConf('field_type', 'text'), 'field_type' => 'text', 'field_match_html' => $this->_formConf('field_match', '^[\u4e00-\u9fa5\a-zA-Z0-9]+$'), 'field_match' => '^[\u4e00-\u9fa5\a-zA-Z0-9]+$', 'is_show' => $conf['address'], 'is_empty' => $conf['is_address']);
				$fields[] = array('field_id' => 0, 'field_name' => '来源渠道', 'field_option' => '', 'field_type_html' => $this->_formConf('field_type', 'text'), 'field_type' => 'text', 'field_match_html' => $this->_formConf('field_match', '^[\u4e00-\u9fa5\a-zA-Z0-9]+$'), 'field_match' => '^[\u4e00-\u9fa5\a-zA-Z0-9]+$', 'is_show' => $conf['origin'], 'is_empty' => $conf['is_origin']);
			} else {
				foreach ($fields as &$f) {
	...
	{
		$data = array();
		$data['wechaname'] = intval($_POST['wechaname']);
		$data['is_wechaname'] = intval($_POST['is_wechaname']);
		$data['tel'] = intval($_POST['tel']);
		$data['is_tel'] = intval($_POST['is_tel']);
		$data['truename'] = intval($_POST['truename']);
		$data['is_truename'] = intval($_POST['is_truename']);
		$data['qq'] = intval($_POST['qq']);
		$data['is_qq'] = intval($_POST['is_qq']);
		$data['is_paypass'] = intval($_POST['is_paypass']);
		$data['portrait'] = intval($_POST['portrait']);
		$data['is_portrait'] = intval($_POST['is_portrait']);
		$data['sex'] = intval($_POST['sex']);
		$data['is_sex'] = intval($_POST['is_sex']);
		$data['bornyear'] = intval($_POST['bornyear']);
		$data['is_bornyear'] = intval($_POST['is_bornyear']);
		$data['bornmonth'] = intval($_POST['bornmonth']);
		$data['is_bornmonth'] = intval($_POST['is_bornmonth']);
		$data['bornday'] = intval($_POST['bornday']);
		$data['is_bornday'] = intval($_POST['is_bornday']);
		...
		if ($custom = $db->where(array('token' => $this->token))->find()) {
			$db->where(array('token' => $this->token))->save($data);
// 			if ($db->where(array('token' => $this->token))->save($data)) {
				$id = $custom['id'];
				$ids = array();
				$t_list = $db_field->field('field_id')->where(array('token' => $this->token, 'set_id' => $id))->select();
				foreach ($t_list as $v) $ids[] = $v['field_id'];
				foreach ($field_name as $k => $val) {
					if (empty($val) && $k == 1) $val = '地址';
					if (empty($val) && $k == 2) $val = '来源渠道';
					if (empty($val)) continue;
					$f_data = array('token' => $this->token, 'set_id' => $id, 'field_name' => $val, 'field_option' => $field_option[$k], 'field_type' => $field_type[$k], 'field_match' => $field_match[$k], 'is_show' => $is_show[$k], 'is_empty' => $is_empty[$k]);
				...
				if ($ids) $db_field->where(array('token' => $this->token, 'set_id' => $id, 'field_id' => array('in', $ids)))->delete();
				$this->success('操作成功');
// 			} else {
// 				$this->error('操作失败');
// 			}
