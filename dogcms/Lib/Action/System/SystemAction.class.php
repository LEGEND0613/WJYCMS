<?php
include 'UpdateFunction.class.php';
/**
 *网站后台
 *@package
 *@author
...
class SystemAction extends BackAction{
	...
	public function _initialize() {
		...
		if (!$this->server_url){
			$this->server_url='http://up8.up88.cc/';
		}
  ...
  PRIMARY KEY (`id`)
	...
	public function index(){
		...
		$url = 'http://update.weidogs.cn/oa/admin.php?m=notice&c=view&a=get_list';
	...
	public function main(){
		...
		*/
        ...
        if($domain_time == '0'){
            $domain_time = '[授权已过期，请联系QQ:3105920978]';
        }else{
            $domain_time='授权版本：(商业版终生使用权)--免费更新服务截止：('.date("Y-m-d",$domain_time).')';
        }
        $this -> assign('ver', $ver);
	...
	public function _needUpdate(){
		...
		$remoteStr=@dogcms_getcontents($url);
		//
	...
	public function doUpdate(){
		...
		if (!isset($_GET['ignore'])){
			if (!is_writable($_SERVER['DOCUMENT_ROOT'].'/dogcms')){
				$cannotWrite=1;
				$this->error('您的服务器dogcms文件夹不可写入，设置好再升级',U('System/main'));
			}
			if (!is_writable($_SERVER['DOCUMENT_ROOT'].'/dogcms/Lib/Action')){
				$cannotWrite=1;
				$this->error('您的服务器/dogcms/Lib/Action文件夹不可写入，设置好再升级',U('System/main'));
			}
		...
		*/
		...
		if (intval($rt['success'])<1){
			...
			$fileStr=@dogcms_getcontents($filename);
			if (!$fileStr){
...
function dogcms_getcontents($url){
	if (function_exists('curl_init')){
		$ch = curl_init();
		$header = "Accept-Charset: utf-8";
		curl_setopt($ch, CURLOPT_URL, $url);
		...
		if($headers['http_code'] == 302){
			return dogcms_getcontents($headers['redirect_url']);
		}
