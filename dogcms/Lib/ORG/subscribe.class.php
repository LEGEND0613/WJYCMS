			...
			if ($cardSet = D('Member_card_set')->where(array('token' => $this->token, 'sub_give' => 1))->find()) {
				if ($card = M('Member_card_create')->field('id, number')->where("token='" . $this->token . "' and cardid=" . intval($cardSet['id']) . " and wecha_id = ''")->order('id ASC')->find()) {
					...
					if (empty($is_card)) {
						...
						$now 	= time();
						M('Userinfo')->where(array('token' => $this->token,'wecha_id' => $this->wecha_id))->save(array('getcardtime' => $now));
						$gwhere = array('token' => $this->token, 'cardid' => $cardSet['id'], 'is_open' => '1', 'start' => array('lt', $now), 'end' => array('gt', $now));
